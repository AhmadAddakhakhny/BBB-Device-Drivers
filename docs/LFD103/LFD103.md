...
---
## Continue kernel journey
1. subscribe to **Stable release mailing list** as maintainers send stable patches for testing.  
```bash
...
```
2. clone the specified tree **linux-5.2.y**
3. copy the distribution config file /boot, setup configuration, compile, install and update-grub
4. reboot, and boot the newly installed kernel
5. Helper script **stable_rc_checkout.sh** made by **Shuah Khan** to use the Stable Release rc git
```bash
	...
	cd linux-$1.y
	#cp /boot/<currentconfig> .config # Replace <currentconfig> Here with the config file you want to use (TBD)
	make -j2 all
	rc=$?; if [[ $rc !=0 ]]; then exit $rc; fi
	su -c "make modules_install install"
	echo Ready for reboot test of Linux-$
```
6. Helper scripts Download the Stable Release Patch and Apply The Patch.
```bash
	# script (1)
	#stable_checkout.sh
	#!/bin/bash
...
```
7. Save logs from the current kernel:
```bash
dmesg_checks.sh
     # !/bin/bash
     #
...
```
8. after rebooting the machine and the kernel doesn't boot, switch to a stable kernel and run kernel selftests ... https://www.kernel.org/doc/html/latest/dev-tools/kselftest.html
```bash
make kselftest
```
### Ways to learn about the kernel
1. preparing to build the documentation 
```bash
# build the docs
make htmldocs # internally checks for the packages dependancies

# OR check for the dependancies drirectly
./scripts/sphinx-pre-install
```
2. Building documents and looking for warnings
```bash
